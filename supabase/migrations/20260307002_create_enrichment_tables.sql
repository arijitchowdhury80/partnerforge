...
CREATE TABLE company_financials (
  ...
  -- Composite PK
  PRIMARY KEY (company_id, audit_id, fiscal_year, COALESCE(fiscal_quarter, 0)),

  -- Composite FK
  FOREIGN KEY (company_id, audit_id) REFERENCES audits(company_id, id) ON DELETE CASCADE,

  -- Validation
  CONSTRAINT financials_quarter_range CHECK (fiscal_quarter BETWEEN 1 AND 4 OR fiscal_quarter IS NULL)
);
