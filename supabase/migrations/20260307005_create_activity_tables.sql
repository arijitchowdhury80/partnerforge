...
CREATE TABLE data_freshness (
  ...
  is_stale BOOLEAN GENERATED ALWAYS AS (next_refresh_at < NOW()) STORED,
...
COMMENT ON COLUMN data_freshness.is_stale IS 'Computed column: true if next_refresh_at has passed';
...
CREATE TABLE enrichment_cache (
  ...
  -- Size tracking
  data_size_bytes INTEGER GENERATED ALWAYS AS (pg_column_size(data)) STORED
);
