/**
 * Fetch CMS Galaxy Data from BuiltWith Lists API
 *
 * Fetches companies using partner CMS technologies and upserts into companies table.
 *
 * Usage:
 *   npx ts-node scripts/fetch-cms-galaxy.ts                    # Fetch all CMS techs
 *   npx ts-node scripts/fetch-cms-galaxy.ts --tech AEM         # Fetch single tech
 *   npx ts-node scripts/fetch-cms-galaxy.ts --dry-run          # Preview without DB writes
 *   npx ts-node scripts/fetch-cms-galaxy.ts --since 30         # Last 30 days only
 */
...
// CMS Technology mappings
// Key = our slug (used in companies.cms_tech column)
// Value = BuiltWith technology name
const CMS_TECHNOLOGIES: Record<string, string> = {
...
async function main() {
  ...
  console.log("CMS GALAXY DATA FETCHER");
  console.log("=".repeat(60));
  ...
  const techsToFetch = targetTech
    ? { [targetTech]: CMS_TECHNOLOGIES[targetTech] }
    : CMS_TECHNOLOGIES;

  if (targetTech && !CMS_TECHNOLOGIES[targetTech]) {
    console.error(`Unknown tech: ${targetTech}`);
    console.log("Available: " + Object.keys(CMS_TECHNOLOGIES).join(", "));
    process.exit(1);
  }
